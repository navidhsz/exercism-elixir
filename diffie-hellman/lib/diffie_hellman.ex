defmodule DiffieHellman do
  ...
  def generate_public_key(prime_p, prime_g, private_key) do
    rem(pow(prime_g, private_key), prime_p)
  end
  ...
  def generate_shared_secret(prime_p, public_key_b, private_key_a) do
    # TODO big number test are disabled.

    rem(pow(public_key_b, private_key_a), prime_p)
  end

  defp pow(n, m) do
    max_concurrency = System.schedulers_online() * 100

    List.duplicate(n, m)
    |> Task.async_stream(& &1, max_concurrency: max_concurrency)
    |> Enum.reduce(fn {:ok, n1}, {:ok, n2} -> {:ok, n1 * n2} end)
    |> elem(1)
  end
end
